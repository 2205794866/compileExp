...
Specifier: TYPE { if ($1[0] == 'i') $$ = new NSpecifier(*(new std::string("int")));
        ...
        | StructSpecifier {$$ = new NSpecifier(*(new std::string("struct"))); $$->line = $1->line;}   
        ;

StructSpecifier: STRUCT OptTag LC DefList RC    {$$ = new NStructSpecifier($2, $4); $$->line = $1;}   
        | STRUCT Tag                            {$$ = new NStructSpecifier($2, nullptr); $$->line = $1;}   
        ;
...
Exp:      Exp ASSIGNOP Exp      {$$=new NAssignment(*(new std::string("ASSIGNOP")), *$1, ASSIGNOP, *$3); $$->line=$1->line;}//$$结点type_id空置未用，正好存放运算符
        ...
        | Exp MINUSASS Exp      {$$=new NAssignment(*(new std::string("MINUSASS")), *$1, MINUS, *$3); $$->line=$1->line;}
        ...
        | Exp MINUSMINUS                        {$$=new NSingleOperator(*(new std::string("MINUSMINUS")), PLUSPLUS, *$1); $$->line=$2; }//这里利用BISON %prec表示和UMINUS同优先级
